...
{
    this->buffer = new Vec3<REAL>[width*height];
    std::memset(this->buffer, 0, sizeof(Vec3<REAL>)*width*height);
}
//------------------------------------------------------------------------------
template<typename REAL>
...
//------------------------------------------------------------------------------
template<typename REAL>
Vec3<REAL>& Framebuffer<REAL>::operator()(int i, int j)
{
...
void Framebuffer<REAL>::ToPPM(const std::string& filename) const
{
    ...
    for (int j = 0; j < height; j++)
    {
        for (int i = 0; i < width; i++)
        {
            ...
            r = this->buffer[idx].X*REAL(255);
            g = this->buffer[idx].Y*REAL(255);
            b = this->buffer[idx].Z*REAL(255);
...
//------------------------------------------------------------------------------
template<typename REAL>
void Framebuffer<REAL>::SetClearColor(const Vec3<REAL>& color)
{
