...
void RenderContext<REAL>::Render()
{
    ...
    for (int i = 0; i < w; i++)
    {
        for (int j = 0; j < h; j++)
        {
            ...
            CGK::Math::Vector3<REAL> direction(x, y, z);
            direction.Normalize();
            
            this->framebuffer.Pixel(i, j) = trace(Ray<REAL>(
                    CGK::Math::Vector3<REAL>(0.0, 0.0, 0.0),
                    direction
                ));
        }
    }
...
//------------------------------------------------------------------------------
template<typename REAL>
CGK::Math::Color3<REAL> RenderContext<REAL>::trace(const Ray<REAL>& ray) const
{
    REAL t = 0.0;
    ...
    {
        const CGK::Math::Matrix4<REAL>& w2o = obj->GetWorldToObject();
        CGK::Math::Vector3<REAL> rayOrigin = w2o*ray.Origin;
        CGK::Math::Vector3<REAL> rayDest = w2o*ray.Direction;
        Ray<REAL> r(rayOrigin, rayDest - rayOrigin);
